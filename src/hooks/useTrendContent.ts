...
export function useTrendContent() {
  ...
  const pollForImages = async (contentId: string) => {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      try {
        visuals =
          typeof data.script_visuals === "string"
            ? JSON.parse(data.script_visuals)
            : (data.script_visuals as unknown as FacelessVisual[]);
      } catch {
        continue;
      }
  ...
  const generate = async (
    ...
    try {
      ...
      toast({
        ...
        description: `${generated.plant_name} — images generating in background.`,
      });
  ...
  const regenerateAllVisuals = async (
    ...
    toast({
      ...
      description: "Refreshed with the latest plate style.",
    });
  };
  ...
  const loadFromHistory = (saved: SavedTrendContent) => {
    setContent({
      id: saved.id,
...
export function useTrendHistory() {
  const [history, setHistory] = useState<SavedTrendContent[]>([]);
  ...
  const fetchHistory = async () => {
    ...
    if (error) {
      console.error("Failed to fetch trend history:", error);
    } else {
      setHistory(
        (data || []).map((item) => {
          ...
          try {
            script =
              typeof item.script === "string"
                ...
                : (item.script as unknown as ScriptStructure);
          } catch {
            script = {
              hook: "",
              dangle_1: "",
          ...
          try {
            thumbnail_prompt =
              typeof item.thumbnail === "string"
                ...
                : (item.thumbnail as unknown as ThumbnailPrompt);
          } catch {
            thumbnail_prompt = {
              mode: "light",
              prompt: item.thumbnail || "",
            };
          }
          ...
          try {
            if (item.script_visuals) {
              faceless_visuals =
                typeof item.script_visuals === "string"
                  ? JSON.parse(item.script_visuals)
                  : (item.script_visuals as unknown as FacelessVisual[]);
            }
          } catch {
            faceless_visuals = [];
          }
          return {
            id: item.id,
            plant_name: item.subject || "Untitled",
