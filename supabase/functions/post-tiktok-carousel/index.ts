...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!TIKTOK_API_KEY) throw new Error('TIKTOK_API_KEY is not configured (connect TikTok)');

    const body = (await req.json()) as Body;
    const images = Array.isArray(body.photo_images) ? body.photo_images.filter((u) => typeof u === 'string' && u.startsWith('http')) : [];
    if (images.length < 2 || images.length > 35) {
      return new Response(JSON.stringify({ error: 'photo_images must contain 2-35 public URLs' }), {
        status: 400,
    ...
    if (!tiktokRes.ok) {
      return new Response(
        JSON.stringify({ error: 'TikTok rejected the request', status: tiktokRes.status, details: json }),
